...
function addTranslatorButton($timelineWrapper, $translateButton) {
  const isStatusPage = /.*twitter.com\/.*\/status\/.*/.test(
    window.location.href,
  );

  let tweetWrapperList = [
    ...$timelineWrapper.querySelectorAll(
      'div[aria-label] article[role=article] div[lang]:not([data-has-translator=true]):not([lang^=zh])',
    ),
  ];
  ...
  if (isStatusPage) {
    const $googleTranslateWrapper = tweetWrapperList.find(
      ($tweetWrapper) => $tweetWrapper.nextElementSibling?.getAttribute('role') === 'button' || $tweetWrapper.nextElementSibling?.querySelector('div > span[aria-expanded]'),
    );
...
function init() {
  ...
  const buttonColor = getComputedStyle(
    document.querySelector('header[role="banner"] a[href="/compose/tweet"]'),
  ...
  $timelineWrapper.addEventListener(
    ...
    (e) => {
      ...
      if (
        ...
        const $textContainer = e.target.closest(
          'div[data-has-translator=true]',
        );
        const $button = $textContainer.getElementsByClassName(
        ...
        chrome?.runtime?.sendMessage && chrome.runtime.sendMessage(
          { type: 'translate', payload: { text, locale } },
          (resp) => {
            if (resp?.type === 'translate-result') {
              $loading.remove();
              isLoading = false;
              $button.classList.add('hide');
              console.log('结果是', resp?.payload?.translateResult);
